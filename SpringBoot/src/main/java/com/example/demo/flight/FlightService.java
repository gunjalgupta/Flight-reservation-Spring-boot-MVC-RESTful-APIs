...
import static org.springframework.data.repository.init.ResourceReader.Type.XML;
...
public class FlightService {
    ...
    @GetMapping
    public @ResponseBody Flight  getFlight(String id){
        return flightRepository.findByFlightNumber(id);
    }


    @PostMapping
    public Flight addFlight(Flight flightDetails, Plane plane, String flightNumber) throws Exception {
        Flight newFlight = null;
        String departure = null, arrival = null;
        DateFormat dateFormat = new SimpleDateFormat("MM/dd/yyyy HH:mm:ss");
        try {
            departure = flightDetails.getDepartureTime();
            arrival = flightDetails.getArrivalTime();
...
//                        HttpStatus.NOT_FOUND);
            ...
            if(flightRepository.findByFlightNumber(flightNumber)!=null)
                return updateFlight(flightDetails, flightNumber,plane);
            flightDetails.setSeatsLeft(plane.getCapacity());
            System.out.println(flightDetails.getSeatsLeft());
            Flight brandNewFlight = new Flight(flightNumber,flightDetails.getPrice(),flightDetails.getOrigin(),flightDetails.getDestination(),flightDetails.getDepartureTime(),flightDetails.getArrivalTime(),flightDetails.getDescription(),flightDetails.getSeatsLeft(),plane);
            newFlight = flightRepository.save(brandNewFlight);
        }
    ...
    @PutMapping()
    public Flight updateFlight(Flight flightDetails, String flightNumber, Plane plane) throws Exception {
        Flight existingFlight = null;
        try{
            existingFlight = flightRepository.findByFlightNumber(flightNumber);
            ...
            existingFlight.setSeatsLeft(flightDetails.getSeatsLeft());
    ...
    @DeleteMapping()
    public @ResponseBody String deleteFlight(String flightNumber) {
        Flight existingFlight = flightRepository.findByFlightNumber(flightNumber);
        if(existingFlight==null){
            return "-2";
        }
            if(existingFlight.getSeatsLeft()==existingFlight.getPlane().getCapacity()){
